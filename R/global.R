...
enet_reg<-function(M,Y,K,eps){
	require(elasticnet)
	cvenet<-try(cv.enet(t(M),(Y),lambda=0.05,s=seq(0,1,length=100),mode="fraction",intercept=FALSE,K=K,plot.it=FALSE,eps=10^-5))
  n<-try(cvenet$index[which.min(cvenet$cv)+max(1,which.min(cvenet$cv[which.min(cvenet$cv):length(cvenet$cv)]<=min(cvenet$cv)+cvenet$cv.error[which.min(cvenet$cv)])-1)-1])
 	model<-try(enet(t(M),(Y),intercept=FALSE,eps=10^-5))
	repu <- predict(model, s=n, type="coef", mode="fraction")
	if(!is.vector(repu)){repu<-rep(0,dim(M)[1])}
	return(repu)
}
