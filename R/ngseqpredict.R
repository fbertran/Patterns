setClass(Class = "ngseqpredict",
representation(nextgenseq_unchanged="nextgen_seq",nextgenseq_changed="nextgen_seq",nextgenseq_predict="nextgen_seq",nv="numeric",network="network",targets="numeric")	)

setMethod("plot",c("ngseqpredict"),function(x,time=NULL,label_v=NULL,frame.color="white",ini=NULL,label.hub=FALSE){
	net<-x@network
	micro<-predict(x@nextgenseq_unchanged,x@network,nv=x@nv)@nextgenseq_predict
	nv<-x@nv
	micro_pred<-x@nextgenseq_predict
	targets<-x@targets
	O<-net@network
  	Omega<-net@network
    O[abs(O)<=nv]<-0
	...
	if(!attr(dev.cur(),"names")=="pdf"){dev.new()}
		...
		sup_pred<-1:dim(micro@nextgenseq)[2]
	sup_pred<-sup_pred[sup_pred%%length(micro@time)==time]
	M<-apply(micro@nextgenseq[,sup_pred]-micro_pred@nextgenseq[,sup_pred],1,mean)
