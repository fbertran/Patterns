...
lasso_reg<-function(M,Y,K,eps,priors,cv.fun=lars::cv.folds
  ...
  n<-try(model$index[which.min(cvlars1$cv)+max(1,which.min(cvlars1$cv[which.min(cvlars1$cv):length(cvlars1$cv)]<=min(cvlars1$cv)+cvlars1$cv.error[which.min(cvlars1$cv)])-1)-1])
