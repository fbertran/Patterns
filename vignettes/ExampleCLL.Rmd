...
```{r loadPatterns, echo=FALSE, eval=TRUE, cache=FALSE}
require(Patterns)
```
```{r retcll, warning=FALSE, cache=TRUE}
require(Patterns)
...
```{r spltcll, warning=FALSE, cache=TRUE}
...
```{r addgeneid, warning=FALSE, cache=TRUE}
...
```{r addgroupselection, warning=FALSE, cache=TRUE, fig.keep='last'}
...
```{r HumanTFDB, warning=FALSE, cache=TRUE}
...
```{r TFinsel, warning=FALSE, cache=TRUE}
...
```{r plotTFinsel, warning=FALSE, cache=TRUE}
matplot(t(selection@microarray[tfs,]),type="l",lty=1)
```
```{r plotTFinsel2, warning=FALSE, cache=TRUE}
