---
title: "Patterns: a modeling tool dedicated to biological network modeling"
...
vignette: >
  %\VignetteIndexEntry{Patterns: a modeling tool dedicated to biological network modeling}
...
<!-- ![Infered F matrix of the network (General shape).](Patterns_files/figure-html/Fresults-1.png) -->



<!-- ![Infered coefficient matrix of the network (General shape).](Patterns_files/figure-html/heatresults-1.png) -->



<!-- ![Infered F matrix of the network (cascade shape).](Patterns_files/figure-html/FresultsLC-1.png) -->



<!-- ![Infered coefficient matrix of the network (cascade shape).](Patterns_files/figure-html/heatresultsLC-1.png) -->



<!-- ![Reverse-engineered network.](Patterns_files/figure-html/plotnet2-1.png) -->



<!-- ![Evolution of a reverse-engineered network with increasing cut-off values.](Patterns_files/animation.gif) -->



<!-- ![Plot of simulated data for cascade networks featuring cluster membership.](Patterns_files/figure-html/plotsimuldata-1.png) -->



<!-- ![Plot of simulated data for cascade networks featuring subject membership.](Patterns_files/figure-html/plotsimuldata-2.png) -->
...
```{r summarysimuldata, cache=TRUE, fig.keep="none"}
summary(M)
```

```{r plotsimuldata, fig.keep='all', cache=TRUE, fig.keep="none"}
plot(M)
```
...
```{r netinfdefault, cache=TRUE, fig.keep="none"}
...
```{r plotfshape2, fig.keep="none"}
plotF(CascadeFshape(4,4),choice="Fshape")
```
```{r plotfshape3, fig.keep="none"}
...
Specific Fshape
```{r netinfLC, cache=TRUE, fig.keep="none"}
...
```{r netinflasso2Weighted, cache=TRUE, fig.keep="none"}
...
```{r FresultsSBWpre, fig.keep="none"}
detach("package:Patterns", unload=TRUE)
library(Patterns)
```

Plot of the inferred F matrix
```{r FresultsSBW, fig.keep="none"}
plot(Net_inf_P_SelectBoostWeighted, choice="F")
```

Heatmap of the coefficients of the Omega matrix of the network
```{r heatresultsSBW, fig.keep="none"}
...
![Evolution of some properties of a reverse-engineered network with increasing cut-off values.](Patterns_files/compare-methods-1.png)
...
```{r cutoff, cache=TRUE, fig.keep="none"}
data("networkCascade")
set.seed(1)
...
Import data.
```{r microselection, warning=FALSE, cache=FALSE}
library(Patterns)
library(CascadeData)
data(micro_S)
...
```{r microselection6, warning=FALSE, fig.keep="none"}
summary(Selection)
```

Plot the final selection:
```{r microselection7, warning=FALSE, fig.keep="none"}
plot(Selection)
```
